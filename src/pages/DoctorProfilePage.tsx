...
const DoctorProfilePage = () => {
  ...
  const doctor = Number.isFinite(doctorId) ? getDoctorById(doctorId, locale) : undefined;
    ...
    return (
      <main className="min-h-screen pt-28 pb-16 px-4">
        <div className="container mx-auto text-center">
          <p className="text-slate-600 mb-6">{t.doctorProfile.notFound}</p>
  ...
  const doctorServices =
    doctor.practiceScope.length > 0 ? doctor.practiceScope : t.doctors.defaultServices;

  const handleWhatsApp = (e: React.FormEvent) => {
    e.preventDefault();
    const selectedService = formData.service || doctorServices[0];
    const tmpl = t.doctorProfile.whatsappTemplate;
    const message = `${tmpl.intro}
${tmpl.patientName} ${formData.patientName}
${tmpl.phone} ${formData.phone}
${tmpl.doctor} ${doctor.name}
${tmpl.service} ${selectedService}
${tmpl.date} ${formData.date}
${tmpl.time} ${formData.time}`;
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-50 pt-24 pb-14">
      <div className="absolute top-0 end-0 h-96 w-96 rounded-full bg-teal-100/60 blur-2xl pointer-events-none" />
      <div className="absolute bottom-0 start-0 h-96 w-96 rounded-full bg-pink-100/60 blur-2xl pointer-events-none" />
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_600px] lg:items-stretch">
          ...
          >
            <section className="relative overflow-hidden border-b border-slate-100 pb-5">
              ...
              <div
                className={`relative flex flex-col gap-5 md:items-center ${
              ...
              >
                ...
                <div className="min-w-0 flex-1 text-start">
                  <div className="mb-3 inline-flex w-fit items-center gap-2 rounded-full bg-teal-50 px-3 py-1.5 text-xs font-bold text-teal-700">
                    <Sparkles size={14} />
                    {t.doctorProfile.medicalFile}
                  </div>
                  <h1 className="text-3xl font-black leading-tight text-slate-900 md:text-4xl">
                    {doctor.name}
                  </h1>
                  <p className="mt-2 text-base font-bold leading-relaxed text-pink-500">
                    {doctor.specialty}
                  </p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <span className="inline-flex items-center gap-2 rounded-full border border-pink-100 bg-white/80 px-3 py-2 text-xs font-bold text-slate-700">
                      ...
                      {t.doctorProfile.servicesCount(doctorServices.length)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <section className="py-5">
              <div className="mb-4 flex items-center gap-3 text-start">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-900">{t.doctorProfile.servicesTitle}</h2>
                  <p className="text-xs text-slate-500">{t.doctorProfile.servicesSubtitle}</p>
                </div>
              </div>
                ...
                <div className="mb-4 flex items-center gap-3 text-start">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-slate-900">
                      {t.doctorProfile.certificatesTitle}
                    </h2>
                    <p className="text-xs text-slate-500">{t.doctorProfile.certificatesSubtitle}</p>
                  </div>
                </div>
          ...
          >
            <div className="flex h-full flex-col overflow-hidden rounded-[2rem] border border-teal-100 bg-white shadow-2xl shadow-teal-100/70">
              <div className="bg-gradient-to-l from-teal-600 to-teal-500 p-7 text-white rtl:bg-gradient-to-r">
                <p className="mb-2 text-sm font-bold text-teal-50">{t.doctorProfile.bookingEyebrow}</p>
                <h2 className="text-3xl font-black">{t.doctorProfile.bookingTitle(doctor.name)}</h2>
                <p className="mt-3 text-sm leading-7 text-teal-50">{t.doctorProfile.bookingDescription}</p>
              </div>
              ...
              <form onSubmit={handleWhatsApp} className="flex flex-1 flex-col space-y-4 p-7">
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-sm font-bold text-slate-700">
                    <User size={16} />
                    {t.doctorProfile.patientName}
                  </label>
                  <input
                    required
                    type="text"
                    placeholder={t.doctorProfile.patientNamePlaceholder}
                ...
                >
                  ...
                  {t.doctorProfile.submitWhatsApp}
                </button>
                <p className="text-center text-xs leading-6 text-slate-500">
                  {t.doctorProfile.submitHint(doctor.name)}
                </p>
              </form>
            </div>
          </motion.aside>
