        ...
        >
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 text-teal-400 font-medium text-sm mb-3">
              <Clock size={18} />
              <span>{t.workingHours.eyebrow}</span>
            </div>
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">{t.workingHours.title}</h2>
            <p className="text-slate-400 text-sm">{t.workingHours.subtitle}</p>
          </div>

          <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 overflow-hidden">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-white/10">
              {t.workingHours.schedule.map((item, index) => (
                <motion.div
                  key={item.day}
                  initial={{ opacity: 0 }}
                  ...
                  className={`flex justify-between items-center gap-4 px-5 py-3.5 bg-slate-900/80 ${
                    item.status === 'closed' ? 'text-red-400' : 'text-white'
                  }`}
                >
            ...
            <div className="flex items-center gap-3 px-5 py-3 bg-teal-500/10 border-t border-white/10">
              ...
              <p className="text-xs text-slate-300">{t.workingHours.note}</p>
            </div>
          </div>
        </motion.div>
      </div>
